...
describe('NewRestaurantForm', () => {
  const restaurantName = 'Mie Ayam Place';
  ...
  describe('when filled up and submitted', () => {
    ...
    it('it does not display a validation error', async () => {
      await fillInForm();
      expect(screen.queryByText(requiredError)).not.toBeInTheDocument();
    });

    it('does not displays a server error', async () => {
      await fillInForm();
      expect(screen.queryByText(serverError)).not.toBeInTheDocument();
    });
  });
